...
import { EmergencyAlert, EmergencyType, EmergencyPriority } from '@/types/emergency';
import { AlertTriangle, Clock, MapPin, User, CheckCircle, XCircle, AlertCircle } from 'lucide-react';
...
export function EmergencyAlertCard({ alert, onRespond, onResolve, onDismiss, canManage = false }: EmergencyAlertCardProps) {
  ...
  return (
    <Card className={`p-4 border-l-4 ${priority.color.replace('bg-', 'border-l-')}`}>
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
          ...
          {alert.location.coordinates && (
            <span className="text-xs">
              ({alert.location.coordinates[0].toFixed(4)}, {alert.location.coordinates[1].toFixed(4)})
            </span>
          )}
        </div>
        ...
        {alert.respondedBy && (
          <div className="p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded">
            <p className="text-sm text-yellow-800 dark:text-yellow-200">
              <strong>Responded by:</strong> {alert.respondedBy} at {new Date(alert.respondedAt!).toLocaleString()}
            </p>
          </div>
        )}
        ...
        {/* Actions */}
        {canManage && alert.status !== 'resolved' && alert.status !== 'false_alarm' && (
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button variant="outline" onClick={() => setIsResolving(false)} disabled={isLoading}>
                        Cancel
                      </Button>
                    </div>
                  </div>
...
interface NewEmergencyAlertFormProps {
  onSubmit: (alert: Omit<EmergencyAlert, 'id' | 'reportedAt' | 'status' | 'notifiedVolunteers'>) => Promise<void>;
  currentUserId: string;
}

export function NewEmergencyAlertForm({ onSubmit, currentUserId }: NewEmergencyAlertFormProps) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <Select
                value={formData.type}
                onValueChange={(value: EmergencyType) => setFormData({ ...formData, type: value })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value: EmergencyPriority) => setFormData({ ...formData, priority: value })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>

...
function printIndexConfigurations(): void {
    ...
    console.log('Navigate to: Database > [Collection] > Indexes > Create Index\n');
...
function generateCLICommands(): void {
    ...
    for (const [collectionId, indexes] of Object.entries(indexConfigs)) {
        ...
        indexes.forEach((config) => {
            ...
            const orders = config.orders ? config.orders.map(order => `"${order}"`).join(' ') : '';
...
function main(): void {
    ...
    console.log('3. Refer to docs/QUERY_OPTIMIZATION.md for detailed instructions\n');
}
